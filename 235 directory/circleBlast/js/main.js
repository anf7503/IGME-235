...
function createLabelsAndButtons() {
    ...
    gameOverScoreLabel.y = sceneHeight - 160;
...
function fireBullet(e) {
    ...
    if (score > 5)
    {
        let b1 = new Bullet(0xFFFFFF, ship.x -  ship.width / 2, ship.y);
...
function gameLoop(){
	if (paused) return; // my time is now!
	...
	for (let c of circles)
    {
        for (let b of bullets)
        {
            ...
            {
                fireballSound.play();
                createExplosion(c.x, c.y, 64, 64); // Here we go!
    ...
    // #6A - Remove Dead Bullets
    bullets = bullets.filter(b=>b.isAlive);

    // #6B - Remove Dead Circles
    circles = circles.filter(c=>c.isAlive);

    // #6B - Remove Explosions
